...
function ProductModal({ product, categories, onClose, onSaved }) {
  ...
  const onSubmit = async (data) => {
    setLoading(true)
    try {
      if (isEdit) await api.put(`/admin/products/${product.id}`, data)
  ...
  const F = ({ label, error, children, half }) => (
    <div className={half ? '' : ''}>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 overflow-y-auto py-8 px-4">
      <div className="bg-white w-full max-w-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-brand-100">
          ...
          <button onClick={onClose} className="text-brand-400 hover:text-espresso text-xl leading-none">&times;</button>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[['is_active', 'Active'], ['is_featured', 'Featured'], ['is_new_arrival', 'New arrival'], ['is_bestseller', 'Bestseller']].map(([key, label]) => (
...
export default function AdminProducts() {
  ...
  const { data: categories } = useQuery({ queryKey: ['admin-categories'], queryFn: () => api.get('/admin/categories').then(r => r.data) })
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => { toast.success('Product deleted'); qc.invalidateQueries(['admin-products']); setDeleteId(null) },
  ...
  const onSaved = () => { setModal(null); qc.invalidateQueries(['admin-products']) }
      ...
      <div className="bg-white border border-brand-100 p-4 flex gap-3">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1) }}
            placeholder="Search by name or SKU…" className="input-field pl-9 py-2.5 text-sm" />
        </div>
      </div>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr key={p.id} className="border-b border-brand-50 hover:bg-brand-50/50 transition-colors">
                ...
                <td className="px-5 py-4">
                  <div>
                    <span className="text-terracotta font-medium">₹{Number(p.sale_price || p.price).toLocaleString('en-IN')}</span>
                    {p.sale_price && <p className="text-[11px] text-brand-300 line-through">₹{Number(p.price).toLocaleString('en-IN')}</p>}
                  </div>
                </td>
                <td className="px-5 py-4">
                  <span className={`text-xs px-2 py-1 ${p.stock === 0 ? 'bg-red-50 text-red-600' : p.stock <= 10 ? 'bg-amber-50 text-amber-700' : 'bg-green-50 text-green-700'}`}>
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    <button onClick={() => setModal(p)} className="text-brand-400 hover:text-terracotta transition-colors">
                      <FiEdit2 size={14} />
                    </button>
                    <button onClick={() => setDeleteId(p.id)} className="text-brand-400 hover:text-red-500 transition-colors">
                      <FiTrash2 size={14} />
                    </button>
                  </div>
                </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="bg-white p-8 max-w-sm w-full text-center">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => deleteMut.mutate(deleteId)} className="flex-1 bg-red-500 text-white text-xs py-3 tracking-widest uppercase hover:bg-red-600 transition-colors">
